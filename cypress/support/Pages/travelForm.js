class travelForm {
    ...
    CallDetails() {
        cy.get('.pqHeader__callUsWrapper a[aria-label="header__callUs"]').first().trigger('mouseover', { force: true });
        cy.get('.toll_wrapper_web', { timeout: 20000 }).invoke('show').should('be.visible')
        cy.wait(1000);
    ...
    checkEndDateselection() {
        ...
        cy.get('a[href="javascript:void(0)"]').click({ multiple: true, force: true })
    }
}
